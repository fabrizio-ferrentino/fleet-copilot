...
/**
 * Demo control plane: inject (or clear) a fault on a device by publishing to the MQTT control
 * topic the simulator listens on, so the UI can break the fleet without a shell. With no deviceId
 * a random online device is picked and returned, so a single button click produces a visible effect.
 */
@RestController
...
public class ControlController {
  ...
  public FaultResponse injectFault(@RequestBody FaultRequest request) {
    ...
    if (!FAULTS.contains(fault)) {
      throw new IllegalArgumentException("unknown fault '" + fault + "', expected one of " + FAULTS);
    }
